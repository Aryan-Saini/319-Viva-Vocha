...
import { exportToPDF3 } from '../utils/pdfExport3';
...
const Presentation3 = () => {
  ...
  const slide = slides3.find(s => s.id === currentSlide);
  const [isFullscreen, setIsFullscreen] = useState(false);

  const goToSlide = useCallback((id: number) => {
    if (id >= 1 && id <= totalSlides) {
      navigate(`/presentation3/${id}`);
    }
  ...
  return (
    <div className={`presentation3-theme ${isFullscreen ? 'fullscreen-mode' : ''}`}>
